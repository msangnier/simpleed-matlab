function model = svmbl1wdualtrain(y, x, varargin)
%SVMBL1WDUALTRAIN bias-regularized SVM with bounded average weight trained in the dual.
%   model = SVMBL1WDUALTRAIN(labels, data, C, lambda, B, norm_weights)
%
%   INPUT:
...
%   - B: lower bound of the average weight B <= sum(w) (default: 0)
...
% Maxime Sangnier (CEA)
% Revision: 0.1 28-May-2014
%           0.2 17-Nov-2014

% Default values
[C, l, B, norm_weights] = check_argin(varargin, 1, 0, 0, ones(size(x, 2), 1));
...
lpf = [ones(N, 1); B]; % Linear application of the objective function
A = [[(x' * Y), ones(n, 1)]; [y', 0]; [(x' * Y), ones(n, 1)]; [y', 0]]; % Linear application of the constraint
lpb = [(1-l) * norm_weights; l; -(1-l) * norm_weights; -l]; % RHS of the constraint
lpe = [-ones(n, 1); -1; ones(n, 1); 1]; % Constraint type (-1 = le, 0 = equal, 1 = ge)
lpl = zeros(N+1, 1); % Lower bound
